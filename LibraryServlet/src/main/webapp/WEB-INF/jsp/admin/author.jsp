<%--@elvariable id="author" type="com.example.library.model.entity.Author"--%>
...
<head>
    ...
    <title>
        <c:choose>
            <c:when test="${mode == 0}">
                <fmt:message key="admin.author.new"/>
            </c:when>

            <c:when test="${mode == 1}">
                <fmt:message key="admin.author.edit"/>
            </c:when>
        </c:choose>
    </title>
...
<div class="container">
    <header class="d-flex flex-wrap align-items-center justify-content-center justify-content-md-between py-3 mb-4 border-bottom">
        ...
        <ul class="nav col-12 col-md-auto mb-2 justify-content-center mb-md-0">
            <li><a href="${pageContext.request.contextPath}/admin/authors" class="nav-link px-2 link-primary"><fmt:message
...
<div class="container col-xxl-5 py-3">
    <div>
        <form action="${pageContext.request.contextPath}/admin/authors/save" method="post" class="needs-validation" novalidate="">

            <input value="${author.id}" type="hidden" name="id">

            <div class="row g-3">
                <div class="col-12">
                    <label for="name" class="form-label"><fmt:message key="author.name"/></label>
                    <input value="${author.name}" type="text" class="form-control" id="name" name="name" required>
                    <div class="invalid-feedback">
                        <fmt:message key="author.name"/>
                    </div>
                </div>
            </div>

            <button class="w-half btn btn-primary btn-lg mt-5" type="submit"><fmt:message key="admin.save"/></button>
            <a class="w-half btn btn-outline-primary btn-lg mt-5" href="${pageContext.request.contextPath}/admin/authors" ><fmt:message key="admin.cancel"/></a>
        </form>
    </div>
</div>
