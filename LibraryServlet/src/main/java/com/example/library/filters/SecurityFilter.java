...
public class SecurityFilter implements Filter {
    ...
    private boolean isAccessOpen(User.ROLE role, String path) {
        ...
        if(role == User.ROLE.GUEST || role == User.ROLE.USER || role == User.ROLE.LIBRARIAN || role == User.ROLE.ADMIN)
            isAccessOpen = path.matches("(/user/(signup|login|logout))");
        if (isAccessOpen) return true;

        if (role == User.ROLE.USER || role == User.ROLE.LIBRARIAN || role == User.ROLE.ADMIN)
            isAccessOpen = path.matches("(/user.*)");
        if (isAccessOpen) return true;
        ...
        if (role == User.ROLE.LIBRARIAN)
            isAccessOpen = path.matches("(/librarian.*)");
